...
def plot(rhr, all_alarms, info, title='', file=None, show=False):
    ...
    # display(all_alarms)
    if not all_alarms is None:
        for d, row in all_alarms.iterrows():
            v = row['alarm']
            if(v > 0):
...
class onlinePloter:
    ...
    def close(self):
        if self.show:
            plt.close(self.fig)
    def plot(self, info, rhr1, test, all_alarms, anomay_score_avg=None):
     if self.show:
      with self.out:
...
# @catch
def plotAll(dataset, all_alarms, file=None, args={},evals=None):
    ...
    missings = rhr24.index[rhr24['heartrate'].isnull()]
    missings_estimate = rhr24.interpolate().loc[missings]
    # ax.plot(missings, missings_estimate['heartrate'], color='white', marker='o', linestyle='', markersize=3, markerfacecolor='gray', markeredgecolor='gray', label="_Imputed Avg RHR \n over night")

    min_ = rhr24['heartrate'].min()
    max_ = rhr24['heartrate'].max()

    ax.plot(rhr24.index, rhr24['heartrate'], color='black',alpha=.6,  marker='',  markersize=2.5, linewidth=1.5, label="Avg RHR")
    ax.plot(rhr24median.index, rhr24median['heartrate'], color='blue', linewidth=1.5, linestyle='dashed', label="Median RHR ")
    # plt.plot(rhr24median.index, rhr24median['heartrate']+3, color='yellow', linewidth=1, linestyle='dashed', label="Med RHR over night + 3")
    # plt.plot(rhr24median.index, rhr24median['heartrate']+4, color='red', linewidth=1, linestyle='dashed', label="Med RHR over night + 4")
    ...
    for m in all_alarms.columns:
        ...
        cols.append(m)
    cols = args['methods'][::-1]
    # cols = [m for m in all_alarms.columns if 'anomaly-detection' not in m]
    for i, method in enumerate(cols):
        # if 'anomaly-detection' in method:continue
        mAlarms = all_alarms[[method]]
        red = mAlarms[mAlarms >= 2].dropna()
        yellow = mAlarms[mAlarms == 1].dropna()
        # print(red)
        for k in red.index:
            ax2.scatter(x=k, y=i, s=mAlarms.loc[k]/mAlarms.max()*30, color='red',label='red alarm')
    ...
    if evals is None:
        ...
        # print(evals.loc['random_0.13']['nature']['tp'])
        ax2.set_yticks(range(len(cols)), labels=[f'{c:<15} TPR={evals.loc[c]["nature"]["TPR"]:.0f} TNR={evals.loc[c]["nature"]["TNR"]:.2f}' for c in cols])


    if(covid_test_date != None):
        # ax.axvspan(covid_test_date, min(rhr24.index[-1], covid_test_date+pd.to_timedelta('14d')), facecolor='red', alpha=0.25, label='Covid')
        ax.axvline(covid_test_date, color='red', zorder=1, linestyle='--',  lw=3, alpha=1, label='Covid Pos. Test')  # Symptom date
    ...
    if(symptom_date != None):
        ax.axvline(symptom_date, color='green', zorder=1, linestyle='--',  lw=3, alpha=1,label='Symptom Onset')  # Symptom date
        # maxSymthom = min(rhr24.index[-1], symptom_date+pd.to_timedelta('14d'))
    ...
    if(window != None):
        ax.axvspan(window[0], window[1], facecolor='green', alpha=0.25)

    # Title & Symptom Onset & Save plot
    # ax.set_xticks(rotation=90)
    ax.set_ylim(int(min_)-1, int(max_)+1)
    h = ax.set_ylabel(f'resting heart rate \nparticipant {info["id"]}')
    # if symptom_date:
    #     ax.set_xlabel('Day relative to symptom')
    # elif covid_test_date:
    #     ax.set_xlabel('Day relative to COVID')
    # else:
    #     ax.set_xlabel('Days')
    h.set_rotation(90)
    ax.set_yticks(np.arange(round10Base(min_), round10Base(max_), 10))
    ...
    if window != None:
        ...
        ref = (covid_test_date-rhr24.index[0]).days #np.where(rhr24.index == covid_test_date)[0][0]
        if symptom_date:
            ref =(symptom_date-rhr24.index[0]).days# np.where(rhr24.index == window[-1])[0][0] 
        # ref = symp_indx if symp_indx!=0 else covid_indx
        

        minor_label = minor_label-np.array([ref])
    ...
    # lgd = ax.legend(prop={'size': 8.5}, bbox_to_anchor=(1.0, 1.0), loc="upper left", frameon=True, facecolor="white")
    lgd = ax.legend(prop={'size': 14}, bbox_to_anchor=(1,-0.4), fancybox=True, ncol=4, shadow=True, loc="lower right", frameon=True, facecolor="white")
    # ax.legend()
    # plt.legend()
...
class eval_ploter:
    ...
    def printEvals(self,total_res):
        total = evals.CalcMetrics(total_res)
    ...
    # @catch
    def plot_evals(self, evals_res,force=False):
      ...
      if self.args.get('methods',None) is not None:
        evals_res=evals_res.loc[np.intersect1d(evals_res.index,self.args['methods'])]
      self.lasttime=pd.Timestamp.now()
      evals_res=evals.CalcMetrics(evals_res)
      
      with self.out:
        clear_output(wait=True)
        #         team_m=team_m.sort_index()
        if self.args.get('filter not similar data count',0):
            evals_res=evals_res[evals_res[('nature','tn')]>=(evals_res[('nature','tn')].max()//3)]
        # evals_res = evals_res.sort_values([('nature', 'tp'),('nature', 'fp')], ascending=[True,False])
        evals_res = evals_res.sort_index(axis=0)
        ind = [i for i, _ in enumerate(evals_res.index)]
        # cols = evals_res.columns.get_level_values(0).drop('total').unique()
  #     team_m=(evals_res.T/evals_res.sum(axis=1)).T
        width = .6
