...
def IsolationForest(dataset,params):
    rhr = dataset['rhrf']
    contamination_rate=params['contamination_rate']
    ...
    train=df['heartrate'].values.reshape(-1,1)
    
    model.fit(train)
    df['alarm'] = pd.Series(model.predict(train)).apply(lambda x: 1 if (x == -1) else 0)
